...
def extract_reviewer_prompt_body(body: str) -> str:
    ...
    that's already just a reviewer prompt (no outer wrapper) works too.
    """
    lines = body.split("\n")
    starts = [i for i, ln in enumerate(lines) if ln.strip() == "---"]
    if len(starts) >= 2:
        inner = "\n".join(lines[starts[0] + 1 : starts[1]]).strip()
        if inner:
            return inner
    return body.strip()
...
def write_session_prompt_file(session_id: str, reviewer_prompt: str) -> Path:
    """Write the substituted reviewer prompt to a per-session file and return its path."""
    SESSIONS_DIR.mkdir(parents=True, exist_ok=True)
    safe_id = re.sub(r"[^\w.-]", "_", session_id or "unknown")
    path = SESSIONS_DIR / f"{safe_id}.md"
    path.write_text(reviewer_prompt)
    return path


def sweep_old_session_files() -> None:
    """Delete session prompt files older than retention. Best-effort; never raises."""
    try:
        if not SESSIONS_DIR.exists():
            return
        cutoff = datetime.now().timestamp() - SESSION_FILE_RETENTION_SECONDS
...
- `prompt`: `Read the file {PROMPT_FILE} and follow the reviewer instructions inside. Return ONLY the JSON verdict specified in that file — no prose before or after, no code fencing.`
...
def build_block_reason(request_text: str, agent_text: str, prompt_path: Path, session_id: str) -> tuple:
    ...
    # Render path with ~ substitution for portability and brevity.
    try:
        rendered_path = str(session_file).replace(str(HOME), "~")
    except Exception:
    ...
    try:
        ...
        audit_log(
            ...
            session_id,
            {
                ...
                "short_reason_chars": len(reason),
                "reviewer_file_chars": session_file.stat().st_size if session_file.exists() else 0,
            },
        )
    except Exception as e:
